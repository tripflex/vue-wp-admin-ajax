...
WPAdminAjax.install = function ( Vue, options ) {
	...
	const defaultOptions = {
		...
		/**
		 * When successFalseReject is set to true, this plugin will reject the request promise when the returned response from WordPress has `success` as `false`, ie: { success: false, data: XXX }
		 * This will be the case whenever you send JSON response with wp_send_json_error() function
		 */
		successFalseReject: true
	}
